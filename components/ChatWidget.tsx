...
import { Send, X, Loader2, MessageSquare, Minimize2 } from "lucide-react";
...
export default function ChatWidget({ chatbotId, chatbotName = "AI Assistant", welcomeMessage, primaryColor = "#6366f1" }: Props) {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[9999] flex flex-col items-end gap-3 chatbot-widget">
      {/* Chat window */}
      {open && (
        <div className="bg-white rounded-2xl shadow-2xl w-80 sm:w-96 flex flex-col overflow-hidden border border-gray-100 animate-fade-in-up" style={{ height: "520px" }}>
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 bg-gray-50">
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                {msg.role === "assistant" && (
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs shrink-0 mr-2 mt-0.5" style={{ backgroundColor: primaryColor }}>
                    🤖
                  </div>
                )}
                <div
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs shrink-0 mr-2 mt-0.5" style={{ backgroundColor: primaryColor }}>🤖</div>
                <div className="bg-white rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm flex gap-1.5">
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="w-2 h-2 bg-gray-400 rounded-full animate-typing-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="p-3 bg-white border-t border-gray-100">
            ...
            <p className="text-center text-[10px] text-gray-300 mt-2">Powered by ChatBase AI</p>
          </div>
        </div>
      )}
